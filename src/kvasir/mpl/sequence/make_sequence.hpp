...
namespace mpl {
    namespace detail {
        ...
        {
            return ((end - 2 * current) < 2) ? end : next_number(current, end / 2); //note that unsigned/2 always rounds down
        }

        constexpr unsigned next_state(unsigned current, unsigned end)
        {
            return ((end - current) < 2) ? end - current : (2 + (next_number(current, end) - 2 * current));
        }
        ...
        struct make_seq_impl<2, C> { //double
            template <unsigned End, unsigned... Is>
            using f = typename make_seq_impl<next_state(2 * sizeof...(Is), End), C>::template f<End, Is..., (Is + sizeof...(Is))...>;
        };
        template <typename C>
        struct make_seq_impl<3, C> { //double +1
            template <unsigned End, unsigned... Is>
            using f = typename make_seq_impl<next_state(2 * sizeof...(Is) + 1, End), C>::template f<End, Is..., (Is + sizeof...(Is))..., (2 * sizeof...(Is))>;
        };
