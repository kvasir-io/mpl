	...
	namespace mpl {
		namespace detail {
			...
			struct push_bst {
				...
				using f = typename push_bst_elem_impl<Comp<T, Elem>::value>::template f<T, Elem>;
			};
			...
			struct push_bst<bst_left<Elem, Left>> {
				template <template <typename...> class Comp, typename T>
				using f = typename push_bst_left_impl<Comp<T, Elem>::value>::template f<Comp, T,
				                                                                        Elem, Left>;
			};
			...
			struct push_bst<bst_right<Elem, Right>> {
				template <template <typename...> class Comp, typename T>
				using f =
				        typename push_bst_right_impl<Comp<T, Elem>::value>::template f<Comp, T,
				                                                                       Elem, Right>;
			};
			...
			struct push_bst_impl<true> {
				...
				using f =
				        bst<BSTElem, typename push_bst<BSTLeft>::template f<Comp, Elem>, BSTRight>;
			};
			template <>
			struct push_bst_impl<false> {
				template <template <typename...> class Comp, typename Elem, typename BSTElem,
				          typename BSTLeft, typename BSTRight>
				using f =
				        bst<BSTElem, BSTLeft, typename push_bst<BSTRight>::template f<Comp, Elem>>;
			};
			template <typename Elem, typename Left, typename Right>
			struct push_bst<bst<Elem, Left, Right>> {
				template <template <typename...> class Comp, typename T>
				using f = typename push_bst_impl<Comp<T, Elem>::value>::template f<Comp, T, Elem,
				                                                                   Left, Right>;
			};
		...
		using stable_sort = typename detail::flatten_bst<mpl::c::call<
		        c::fold_left<detail::push_func<Comp>>, List, nothing>>::f;

		namespace c {
			template <template <typename...> class Comp, typename C = listify>
			struct stable_sort {
				template <typename... Ts>
				using f = c::call<C, mpl::stable_sort<list<Ts...>, Comp>>;
			};
		}
	}
}
