...
namespace kvasir {
	namespace mpl {
		template<typename C = identity>struct is_void {
			template<typename T>
			using f = typename C::template f<bool_<std::is_void<T>::value>>;
		};
		template<typename C = identity>struct is_null_pointer {
			template<typename T>
		...
		template<typename C = identity>struct is_pod {
			template<typename T>
			using f = typename C::template f<bool_<std::is_pod<T>::value>>;
		};
		template<typename C = identity>struct is_literal_type {
			template<typename T>
		...
		template<typename C = identity>struct is_polymorphic {
			template<typename T>
			using f = typename C::template f<bool_<std::is_polymorphic<T>::value>>;
		};
		template<typename C = identity>struct is_abstract {
			template<typename T>
			using f = typename C::template f<bool_<std::is_abstract<T>::value>>;
		};
		template<typename C = identity>struct is_final {
			template<typename T>
			using f = typename C::template f<bool_<std::is_final<T>::value>>;
		};
		template<typename C = identity>struct is_signed {
			template<typename T>
			using f = typename C::template f<bool_<std::is_signed<T>::value>>;
		};
		template<typename C = identity>struct is_unsigned {
			template<typename T>
			using f = typename C::template f<bool_<std::is_unsigned<T>::value>>;
		};
		template<typename C = identity>struct is_constructible {
			template<typename T>
			using f = typename C::template f<bool_<std::is_constructible<T>::value>>;
		};
		template<typename C = identity>struct is_trivially_constructible {
			template<typename T>
			using f = typename C::template f<bool_<std::is_trivially_constructible<T>::value>>;
		};
		template<typename C = identity>struct is_nothrow_constructible {
			template<typename T>
		...
		template<typename C = identity>struct is_trivially_default_constructible {
			template<typename T>
		...
		//};
		template<typename C = identity>struct is_base_of {
			template<typename T, typename U>
			using f = typename C::template f<bool_<std::is_base_of<T, U>::value>>;
		};
		template<typename C = identity>struct is_convertible {
			template<typename T, typename U>
		...
		template<typename C = identity>struct make_signed {
			template<typename T>
			using f = typename C::template f<typename std::make_signed<T>::type>;
		};
		template<typename C = identity>struct make_unsigned {
			template<typename T>
			using f = typename C::template f<typename std::make_unsigned<T>::type>;
		};
		template<typename C = identity>struct remove_extent {
			template<typename T>
			using f = typename C::template f<typename std::remove_extent<T>::type>;
		};
		template<typename C = identity>struct remove_all_extents {
			template<typename T>
			using f = typename C::template f<typename std::remove_all_extents<T>::type>;
		};
		template<typename C = identity>struct decay {
			template<typename T>
			using f = typename C::template f<typename std::decay<T>::type>;
		};
		template<typename C = identity>struct common_type {
			template<typename T>
			using f = typename C::template f<typename std::alignment_of<T>::type>;
		};
		template<typename C = identity>struct underlying_type {
			template<typename T>
			using f = typename C::template f<typename std::alignment_of<T>::type>;
		};	
	}
